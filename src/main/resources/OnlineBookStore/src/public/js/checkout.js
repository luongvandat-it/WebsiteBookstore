...
$(document).ready(function () {
    ...
    $("#btnConfirmPay").click(async function () {
        ...
        await $.ajax({
            ...
            success: async function (data) {
                console.log(data);
                orderId += data;
                localStorage.setItem("orderIdTemp", orderId);
                let listBook = JSON.parse(localStorage.getItem("books"));

                // save order details
                // alert(listBook.length)
                for (let i = 0; i < listBook.length; i++) {
                    console.log('lan', i, 'chua vao')
                    console.log(listBook[i].status)
                    if (listBook[i].status) {
                        console.log('lan', i)
                        ...
                        console.log(localStorage.getItem("orderIdTemp").toString())
                        await $.ajax({
                ...
                for (let j = 0; j < listBook.length; j++) {
                    console.log('chua xoa lan ',j);
                    if (!listBook[j].status) {
                        listBook2.push(listBook[j]);
                    }
                }
                console.log(listBook2)
                localStorage.removeItem("books");
                localStorage.setItem("books", JSON.stringify(listBook2));
